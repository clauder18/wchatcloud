...
backend_redis_create() {
  ...
  sudo su - root <<EOF
  usermod -aG docker deploywchatcloud
...
backend_start_pm2() {
  ...
  sudo su - deploywchatcloud <<EOF
  cd /home/deploywchatcloud/wchatcloud/backend
  pm2 start wchatcloud/server.js --name wchatcloud-backend
EOF
...
cat > /etc/nginx/sites-available/wchatcloud-backend << 'END'
server {
...
ln -s /etc/nginx/sites-available/wchatcloud-backend /etc/nginx/sites-enabled
EOF
