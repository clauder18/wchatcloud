...
system_create_user() {
  print_banner
  printf "${WHITE} 💻 Agora, vamos criar o usuário para deploy WchatCloud...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2
 
  sudo su - root <<EOF
  useradd -m -p $(openssl passwd $deploy_password) -s /bin/bash -G sudo deploywchatcloud
  usermod -aG sudo deploywchatcloud
  mv "${PROJECT_ROOT}"/wchatcloud.zip /home/deploywchatcloud/
EOF
...
system_node_install() {
  ...
  sleep 2
  sudo -u postgres psql -c "ALTER USER postgres PASSWORD 'psql#tdcenter';"
  sudo -u postgres psql -c "CREATE DATABASE wchatcloud;"
  exit
EOF
...
debian_docker_install() {
  ...
  curl -fsSL https://download.docker.com/linux/focal/gpg | gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg
  
  echo \
    "deb [arch=amd64 signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/focal \
...
system_pm2_install() {
  ...
  sudo su - root <<EOF
  npm install -g pm2
  pm2 startup ubuntu -u deploywc
  env PATH=\$PATH:/usr/bin pm2 startup ubuntu -u deploywchatcloud --hp /home/deploywchatcloud
EOF
...
cat > /etc/nginx/conf.d/wchatcloud.conf << 'END'
